"""GitHub - Core Exception Hierarchy (SAP-042)
...
class GithubError(Exception):
    """Base exception for all GitHub errors.

    All domain-specific exceptions inherit from this base class,
    making it easy to catch all GitHub-related errors.
...
class GithubTimeoutError(GithubError):
    ...
    def __init__(
        ...
        super().__init__(message=message, code="TIMEOUT", details=details)
...
def is_github_error(exc: Exception) -> bool:
    """Check if exception is a GitHub error.

    Args:
        exc: Exception to check

    Returns:
        True if exception is a GitHubError or subclass
    """
    return isinstance(exc, GithubError)
...
def get_error_code(exc: Exception) -> str:
    ...
    Returns:
        Error code string, or "UNKNOWN_ERROR" if not a GitHubError
    """
...
def get_error_details(exc: Exception) -> dict[str, Any]:
    ...
    if isinstance(exc, GithubError):
        return exc.to_dict()
    return {"error": type(exc).__name__, "message": str(exc), "details": {}}
