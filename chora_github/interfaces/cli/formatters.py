"""GitHub - CLI Output Formatters (SAP-043)
...
class GithubJSONEncoder(json.JSONEncoder):
    """Custom JSON encoder for GitHub types."""
...
def format_json(data: Any, pretty: bool = True) -> str:
    ...
    return json.dumps(data, cls=GithubJSONEncoder, indent=indent, ensure_ascii=False)


def format_table(
    data: list[dict[str, Any]], columns: list[str], headers: dict[str, str]
) -> str:
...
def print_entity_list(
    entities: list[dict[str, Any]], format: OutputFormat, columns: list[str] = None
):
