...
**Architecture**: Multi-Interface (CLI, REST, MCP)
...
- ✅ **MCP**: Model Context Protocol for AI assistants (Claude Desktop, etc.)

**Primary use case**: GitHub integration server providing issues, pull requests, and repository file management across multiple interfaces
...
→ Uses github:list_issues with repo=octocat/Hello-World, state=open

"Create issue 'Bug fix needed' in my-org/my-repo"
→ Uses github:create_issue with title and repo parameters

"Show me README.md from octocat/Hello-World"
→ Uses github:get_file_contents with path=README.md
```

**See**: [API.md](API.md) MCP Interface section for complete tool reference
